...
winner gState
  | or (map (all (== X)) winPos) = X
  | or (map (all (== O)) winPos) = O
  | otherwise = N
  where 
...
isFinal gState = isFilled gState || winner gState /= N
  where 
    isFilled gState = all (/= N) $ concat (board gState)
score :: GameState -> Int 
score gState
...
bestState gState depth = state
  where 
    (maxScore, state) = foldl1 max $ zip nextScores nextStates' 
    ...
    negamax depth gState 
      ...
      where 
        bestScore = foldl1 max nextScores 
...
startGame = do
  ...
  if winner' == N 
     ...
     else putStrLn $ (show winner') ++ " Won the Game."
main :: IO ()
main = do
   startGame